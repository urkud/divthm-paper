...
\begin{abstract}
  ...
  to prove Cauchy-Goursat theorem (for some simple domains) and
...
For \(C^{1}\) smooth functions \(f\), \(g\), the equality immediately
follows the Fundamental Theorem of Calculus and the Fubini Theorem.
...
\href{https://en.wikipedia.org/wiki/Cauchy's_integral_theorem}{Cauchy's
  integral theorem} (a.k.a. Cauchy-Goursat theorem) for a rectangle.

There is a gap in the proof outlined above: I explained how to prove
the Green's formula for a \emph{continuously differentiable} function
but the Cauchy-Goursat theorem works for any \emph{complex
  differentiable} function. A common misbelieve is that this makes
Green's formula (and divergence theorem) unusable for Cauchy-Goursat
theorem, and one has to prove it using an explicit infinite descend.
...
in~\cite{TODO} that works for a vector field with \emph{continuous
  divergence}, though the actual proof I formalized is very different.
...
provers (e.g., the Green's formula was formalized in
Isabelle~\cite{TODO} and large chunks of complex analysis were
formalized in Isabelle~\cite{TODO} and HOL-Light~\cite{TODO}).
However, as far as I know, this is the first project where the
Cauchy-Goursat theorem is formalized as a corollary of the divergence
theorem. Also, the assumptions in the Cauchy-Goursat theorem are
slightly weaker than in most textbooks.

\section{Structure of the paper}%
\label{sec:structure-paper}

In \autoref{sec:gener-diverg-theor} I informally discuss different
ways to formalize the Green's theorem mentioned above. TODO

\section{Generalizations of the divergence theorem}%
\label{sec:gener-diverg-theor}

The divergence theorem for \(C^{1}\) vector fields on rectangular
boxes can be generalized in a few different directions, leading to
several theorems none of which implies the others.

\paragraph*{Shape of the domain}
One natural direction of generalization is to deal with
non-rectangular domains. Usually, Green's formula is stated for a
union of elementary regions. This generalization is done (in dimension
two) in Isabelle several years ago, see~\cite{Abdulaziz_Paulson} for
details. I only deal with rectangular boxes, so I am not going to
discuss this in any more details.

% https://www.cl.cam.ac.uk/~lp15/papers/Formath/Greens-theorem.pdf

\paragraph*{More general codomain}
...
\paragraph*{Integrable partial derivatives}
It is easy to see that the standard proof based on Fubini theorem and
...
formalized the other one because it implies Cauchy--Goursat theorem.

TODO: formalize this one if I have a spare evening before the deadline.

\paragraph*{Fréchet differentiability}
Recall that a function \(f\colon E_{1}\to E_{2}\) between normed
vector spaces is called \emph{Fréchet differentiable} at a point \(x\)
with derivative given by a continuous linear map
\(f'\colon E_{1}\to E_{2}\) if \(f(x+y)=f(x)+f'(y)+o(y)\) as
\(y\to 0\). Clearly, a Fréchet differentiable function \(f\colon \)has all partial
derivatives and they are equal to the values of \(f'\) on the basis
vectors.

In 1981, Mawhin~\cite{Mawhin81} introduced a generalization of
...
\begin{theorem}%
  ...
  its integral equal to the difference of the integrals of \(f\) over
...
\paragraph*{Weaker assumptions on a subset of the box}
...
recall the relevant definitions and theorems.

\subsection{Henstock-Kurzweil integral in dimension one}\label{sec:HK-integral-dim1}
First recall the definition of the one-dimensional Henstock-Kurzweil integral.
...
Slight modifications of this definitions give the integrals of Riemann
...
\begin{definition}
  The \emph{open box} \((a, b)\) is the product of open intervals
  \((a_{i}, b_{i})\):
...
this definition (and any other generalization of Henstock-Kurzweil
integral that satisfies Fubini theorem, see~\cite{Pfeffer93}), so we
need another generalization to prove the divergence theorem.

There are several generalizations of the Henstock-Kurzweil such that
the divergence of any Fréchet differentiable function is integrable.
We use the one suggested by Mawhin~\cite{Mawhin81}.
...
Mathlib project uses the Bochner integral as its main
integral. A~Bochner integrable function is McShane integrable (hence,
...
\begin{theorem}%
  ...
  topology\footnote{This is a technical assumption required because of
    the way Bochner integral is defined in mathlib.}. Let
  \(I=(a,b]\subset\bbR^{n}\) be an open-closed box.
...
\label{sec:GP-impl}
The following notation is heavily used in the code below and is
specific to \texttt{mathlib} and/or this project.
...
The membership of a vector \lstinline{x : ℝⁿ} in a box is defined by

\begin{lstlisting}
instance : has_mem (ℝⁿ) (box n) :=
⟨λ x I, ∀ i, x i ∈ Ioc (I.lower i) (I.upper i)⟩
\end{lstlisting}
and the order is the inclusion order on the corresponding sets.
...
represented as the bottom element of \lstinline{⊥ : with_bot (box n)},
where \lstinline{with_bot α} is the type \lstinline={⊥} ∪ α=,
implemented as a type synonym for \lstinline{option α} with custom order.

The type \lstinline=with_bot (box n)= is a lattice, where the meet of
two boxes is there intersection and the join of two boxes is the
minimal box that includes both of them.

For an open-closed box \lstinline{I : box n}, \lstinline{I.Ioo} and
\lstinline{I.Icc} are the corresponding open and closed boxes,
respectively.

Given a box \lstinline=I : box (fin (n + 1))= and an index
...
embedding with leaving a \enquote{hole} at \(i\): it sends \(j<i\) to
\(j\) and \(j\ge i\) to \(j+1\). I also define two embeddings
\lstinline=I.front_face i, I.back_face i : ℝⁿ → ℝⁿ⁺¹= that insert
\lstinline=I.upper i= and \lstinline=I.lower i=, respectively, as the
\(i\)-th coordinate. These embeddings map \lstinline=I.face i= to two
the faces \lstinline+x i = I.upper i+ and \lstinline+x i = I.lower i+
of \(I\).

\subsection{Partitions}\label{sec:partitions}

Recall that a \emph{partition} of a box \(I\) is a finite collection
of pairwise disjoint boxes that cover \(I\). Sometimes it is useful to
deal with a collection of pairwise disjoint boxes that cover only a
part of \(I\), so I define a \emph{prepartition} of a box
...
def prepartition.is_partition (π : prepartition I) : Prop :=
∀ x ∈ I, ∃ J ∈ π, x ∈ J
\end{lstlisting}
...
lemma box.exists_tagged_partition_is_Henstock_is_subordinate_homothetic
  (I : box n) (r : ℝⁿ → ℝ>0) :
...
Then I use it to prove that for any prepartition there exists a
refinement of this prepartition with the same distortion and a choice
of Henstock tagged prepartition that is subordinate to a given gauge
function, covers the same part of the original box, and has the same
distortion.

\begin{lstlisting}[caption={Cousin's lemma, version 2}]
lemma prepartition.exists_tagged_le_is_Henstock_is_subordinate_Union_eq
  (r : ℝⁿ → ℝ>0) (π : prepartition I) :
...
\begin{tabular}{rp{10cm}}
  ...
  \lstinline=bDistortion=&this is a regular integral, the gauge function may depend on the distortion of a partition;
\end{tabular}
...
(e.g., Henstock-Sacks inequality, see below) uniformly for all these
integrals. On the other hand, it is hard to add more integrals (e.g.,
those defined in~\cite{TODO}) to the hierarchy.
...
but I decided to cut off a corner here and introduce this assumption
instead adding an explicit assumption \(c>1\) here and there and of
...
\begin{lstlisting}[caption={Definition of the box integral}]
...
lemma has_integral_GP_pderiv (f : ℝⁿ⁺¹ → E) (f' : ℝⁿ⁺¹ → ℝⁿ⁺¹ →L[ℝ] E)
  ...
  (i : fin (n + 1)) :
  has_integral I GP (λ x, f' x (pi.single i 1)) dV
    (integral (I.face i) GP (f ∘ I.front_face i) dV -
      integral (I.face i) GP (f ∘ I.back_face i) dV)
\end{lstlisting}
Here \lstinline=pi.single i 1= is the \(i\)-th basis vector in
\(\bbR^{n+1}\). The actual statement also uses a longer name instead
of \(dV\).
...
lemma has_integral_GP_divergence_of_forall_has_deriv_within_at
  ...
  (Hs : ∀ x ∈ s, continuous_within_at f I.Icc x)
  (Hd : ∀ x ∈ I.Icc \ s, has_fderiv_within_at f (f' x) I.Icc x) :
  has_integral I GP (λ x, ∑ i, f' x (pi.single i 1) i) dV
...
imply that the difference between the difference of the integrals of
...
The Lean mathematical library uses the Bochner integral as its main
definition of an integral. This integral is a generalization of the
Lebesgue integral, see~\cite{TODO} for details. In order to transfer
the result from the GP-integral to the Bochner integral, I prove that
any Bochner integrable function is integrable in the sense of any box
integral with \lstinline~bRiemann = ff~. In other words, a Bochner
integrable function is integrable in the sense of the McShane,
Henstock-Kurzweil, and GP-integrals.
...
The proof follows~\cite{TODO}, with some modifications required to
...
\begin{lstlisting}
lemma aux (I : box (fin (n + 1))) (f : ℝⁿ⁺¹ → Eⁿ⁺¹) (f' : ℝⁿ⁺¹ → ℝⁿ⁺¹ →L[ℝ] Eⁿ⁺¹)
  (s : set ℝⁿ⁺¹) (hs : countable s)
...
In the following statement, I heavily use local notation to make it
more readable.
...
\begin{lstlisting}[caption={Cauchy-Goursat theorem for a rectangle},label=lst:cauchy-rect]
lemma integral_boundary_rect_eq_zero_of_differentiable_on_off_countable
  (f : ℂ → E) (z w : ℂ) (s : set ℂ) (hs : countable s)
...
\begin{lstlisting}[caption=Cauchy-Goursat theorem for an annulus,label=lst:cauchy-annulus]
...
Applying this theorem to \(\frac{f(z)}{z-c}\) and taking the limit as
\(r\) tends to \(0\) from the right, I prove the Cauchy integral
formula for the value of a complex differentiable function at the
center of a disk.
...
Applying this lemma to the function \((z-c)f(z)\), we prove the
Cauchy-Goursat theorem for a disk.

\begin{lstlisting}[caption=Cauchy-Goursat theorem for a disk,label=lst:cauchy-circle]
...
be differentiable, then the same formula from the previous case by
continuity.
...
and is continuous on the corresponding closed disk is analytic on the
open disk.
...
TODO: removable singularity, max modulus
...
survey~\cite{TODO}. Some of these theorems allow the function to be
continuous, not differentiable, on a countable set of
...
to do that as a part of a course. The generalization will be done by
summer, either by the student or by myself.

TODO say that HOL-Light has a library about multivariable complex analysis.
...
already formalized the removable singularity theorem and the maximum
modulus principle but clearly this is just the beginning.
