...
\item the divergence theorem for the Bochner integral\footnote{The
    Bochner integral is a generalization of the Lebesgue integral to
    functions that take values in Banach spaces, see
    \autoref{ssec:Bochner} for more details.}, see
...
\begin{definition}
  ...
  \(\frac{b_{i}-a_{i}}{b_{j}-a_{j}}\) over all \(i, j\).
...
integrable (hence Henstock-Kurzweil and GP-integrable)\todo{define
  McShane integrability in higher dimension?}, thus
...
of the partition are homothetic to the original box.\todo{Explain}
...
add more integrals to the collection\todo{List some}.
...
\lstinline=split= introduced above.\todo{Explain}
\begin{lstlisting}
...
of overengineering.\todo{Explain}
...
\emph{coordinate hyperplanes}. These theorems can be transfered to
more general versions of the Cauchy-Goursat theorems.
